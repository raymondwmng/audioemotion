...
from fea_data_dom import fea_data_npy
from fea_data_dom import fea_test_data_npy
import sys
import os
import shutil
import glob
...
#            train_ref += valid_ref
            ...
            validset = fea_data_npy(valid_fea, BATCHSIZE, traindatalbl)
            valid_dataitems=torch.utils.data.DataLoader(dataset=trainset,batch_size=1,shuffle=False,num_workers=2)
        trainset = fea_data_npy(train_fea, BATCHSIZE, traindatalbl)
...
#            test_ref = database[datalbl][EXT]['test']['ref']
            testset = fea_test_data_npy(test_fea, datalbl, traindatalbl)
