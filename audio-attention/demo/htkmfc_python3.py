...
class HTKFeat_read(object):
    ...
    def readheader(self):
        ...
        if self.parmKind & _C:
            self.dtype = 'h'
#            self.veclen = self.sampSize / 2 # TypeError: integer argument expected, got float
            self.veclen = int(self.sampSize / 2)
            if self.parmKind & 0x3f == IREFC:
                self.A = 32767
                self.B = 0
            else:
                self.A = numpy.fromfile(self.fh, 'f', int(self.veclen))
                self.B = numpy.fromfile(self.fh, 'f', int(self.veclen))
                if self.swap:
    ...
    def getall(self, filename):
        ...
        self.readheader()
        self.seek(0)
        data = [[]]*(self.nSamples-4)
        for i in range(self.nSamples-4):
            data[i] = self.readvec()
        return numpy.array(data)

    # Rosanna added 
    def getsegment(self, filename, beg, end):
        self.open_file(filename)
        self.seek(beg)
        end = min(end,self.nSamples-4)
        data = [[]]*(end-beg)
#        print(len(data), beg, end, self.nSamples-4)
